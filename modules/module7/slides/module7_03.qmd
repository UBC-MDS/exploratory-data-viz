...
points.add_params(click_legend)
```
...
by binding `selection_point` to it.
...
(points & bars).add_params(click_legend) 
```
...
hover = alt.selection_point(fields=["state"], on="mouseover")
choropleth = (
    alt.Chart(state_map)
    .mark_geoshape()
    .transform_lookup(
...
We also added a `selection_point` interaction,
...
```{python}
choropleth & points.encode(tooltip='state').add_params(hover)
```
