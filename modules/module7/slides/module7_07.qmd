...
Use the vegafusion data transformer by including the following line at the start of your analysis:
...
:::{.notes}
Here, we're using `alt.data_transformers.enable('vegafusion')`, which differs from what's shown in the video. That's because since Altair version 5, `vegafusion` is the most effective way to work with large datasets and the `data_server` shown in the video is no longer supported.

Vegafusion reduces the size of the data passed to altair, by removing unused columns and performing data transformations (e.g. binning and aggregations) before Altair receives the data. You can [read more in the Altair documentation](https://altair-viz.github.io/user_guide/large_datasets.html#vegafusion-data-transformer).
