...
we first need to create a selection parameter.
...
After creating the selection parameter,
we add it to our chart using `add_params()`,
...
points = (alt.Chart(cars).mark_circle().encode(
    ...
    alt.Y('Miles_per_Gallon', title='Fuel efficiency (miles / gal)'),
    color=alt.condition(brush, 'Origin', alt.value('lightgray')))
.add_params(brush)).properties(height=225, width=300)
