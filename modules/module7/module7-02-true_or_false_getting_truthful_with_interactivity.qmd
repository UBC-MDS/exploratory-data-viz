...
<script>
    generateQuiz(
        ...
        {
        'True': 'Try again! Brush selections are added via <code>.add_params()</code> and  <code>.selection_interval()</code>, not the <code>.interactive()</code> method.',
        'False': ''
        },
        'False',
    );
...
- Are you setting `brush` in  `add_params`?
