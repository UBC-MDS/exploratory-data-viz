...
<script>
    generateQuiz(
        ...
        { 
        '<code>"subject", row=3</code>': '𝗿𝗼𝘄 needs a column name as an argument value',
        '<code>"subject", rows=4</code>': 'This would produce 4 rows.',
        '<code>"subject", columns=4</code>': '',
        '<code>"subject", column=4</code>': 'The 𝗰𝗼𝗹𝘂𝗺𝗻 argument needs a column label as an input.'
        },
        '"subject", columns=4',  
    );
</script>
...
solution_stripped = strip_bin(solution_dict)
result_stripped = strip_bin(result_dict)

assert_chart_equal(solution_stripped, result_stripped)
```
