...
<script>
    generateQuiz(
        ...
        { 
        'True': '',
        'False': 'Setting a colour channel to the same column as the facet will change each of the faceted plots to different colours.'
        },
        'True',  
    );
</script>
...
<script>
    generateQuiz(
        'mcq4', 
        'Question 2',  
        'If we wanted to facet the plot below by the column 𝘀𝘂𝗯𝗷𝗲𝗰𝘁 and the number of plots per row at 4, what would be needed instead of "???" in the code below?',  
        { 
        '"subject", row=3': '𝗿𝗼𝘄 needs a column name as an argument value',
        '"subject", row=4': 'This would produce 4 rows.',
        '"subject", columns=4': '',
        '"subject", column=4': 'The 𝗰𝗼𝗹𝘂𝗺𝗻 argument needs a column label as an input.'
        },
        '"subject", columns=4',  
    );
</script>
