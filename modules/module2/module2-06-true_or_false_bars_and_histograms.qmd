...
<script>
    generateQuiz(
        'mcq4', 
        'Question 2',  
        'If we wanted to increase the number of bars in the histogram (see code below), what would be needed instead of "???" in the code below?',  
        { 
        '50': 'You need to use a helper function to increase the number of bars and not just set `bin` equal to a number.',
        'alt.Bin(50)': 'There needs to be some sort of argument set within `alt.Bin()`.',
        'bin=alt.Bin(bins=30)': 'You are close but the argument `bins` within the helper function `alt.Bin()` is not quite right.',
