...
We can already see what the most commons values are for each column,
...
Notes:
To answer how the counts are distributed between different categorical values,
...
and fall within the R and PG-13 ratings.

We could have included the title column here to check that no two movies have the same title.
After that, we can safely skip that subplot since it is rather messy with hundreds of titles.
...
so we only look at the one plot below the diagonal.
In this case, it means we could have just made a single plot,
but if you have more than two categories to repeat over,
there would be additional plots under the diagonal.
...
as some analysis methods are not robust against samples sizes that are this unequal
and we need to adapt our analysis strategy accordingly
