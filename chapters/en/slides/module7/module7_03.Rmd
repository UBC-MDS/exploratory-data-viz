...
We used `alt.selection_single` and `alt.condition` to make it  interactive,
...
If we wanted to select multiple genres in the legend,
we could have used `alt.selection_multi` instead of `alt.selection_single`.
...
We then use both these widgets to the `selection_single` object,
...
via setting `init={'IMDB_Rating': 4})` in `alt.selection_single`.
