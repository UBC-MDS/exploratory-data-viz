...
When filtering on a date using Pandas,
...
As previously,
we can clarify what we are visualizing by setting a title which we will demonstrate in the next slide. 
...
To sort the bars,
we will use the arguments in the helper functions `alt.X` and `alt.Y`.
...
Note that you will not get the exact number of bins that you set in `maxbins` and instead, 
