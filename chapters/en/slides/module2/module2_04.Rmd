...
than the sample dataset we saw in the first module.
We will be exploring this data in the assignments for the first two modules.

In later labs
we will look at many other datasets,
but we're sticking to a familiar one for now
so that we can focus on laying down a solid understanding
of the visualization principles with data that we already know.
...
Notes:
As we read in the data with `pandas`,
we specify to read in the `year` column as a date,
...
so make sure to parse these columns as dates when using pandas.
...
without worrying about that the entire dataframe will be included!
...
so we need to help it by indicate the datatype.

This is what the `:T` and `:Q` after the column names does
...
(that you also saw in the Programming with Python course).
...
for example colors, fruits, or countries.
...
Now that we have seen what the datatabel looks like,
and which data types the values in each column are,
...
<center><img src="/module2/drawing.png"  width = "80%"></center>
...
Next, we try to plot it in Altair using `mark_line()` and maping `year` to the x-axis and `population` to the y-axis. 
...
For now on we will just use it to clarify
