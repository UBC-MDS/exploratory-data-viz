...
when we when encode a dataframe variable as the `color` channel. Notice we are now using the helper functions we saw before but now with `alt.Color`. (This must be spelt without the "U")
...
Instead of using the `columns` parameter, we could specify one categorical dataframe variable each
...
whereas those with a high life expectancy
...
Notes:
So far we have encoded the same dataframe variable in both the facet and colour channels.
...
into groups by colouring according to another dataframe variable.

This would allow us to answer questions of similar complexity 
...
This visualization reveals that there is lower child mortality, fewer children per woman,
...
If there were so many categories that the colours become hard to tell apart,
...
we could create a stacked bar chart.

Layering wouldn't make sense here either,
as that would just hide the bars behind each other.
What we would want to do is adding a small offset to each colour
so that they dodge each other and are placed side by side.

This offset functionality does not yet exist in Altair,
...
and encoding the same dataframe variable in both the y and colour channel,
...
We have removed the redundant colour legend
in favour of the direct labels on the y-axis.
We will talk more about direct labelling versus legends in a future module,
but in general,
direct labelling is often clearer.
...
it is often useful to use colour for the most important comparison,
