...
1. Calculate the bell-shaped bins and add them together as in the animation as few slides ago.
...
which we here call `'density'`.

Since the `'density'` variable
is not part of the pandas dataframe,
Altair cannot ask pandas which data type it is.
Therefore,
we need to add `':Q'` to indicate that the density has quantitative values.

We could also have calculated the sum of bins outside Altair 
