...
Notes:
We have seen that we can use facetting for histograms
...
Notes:
For this exercise we will use a new movies dataset that this time contains more genres.
The question we will try to answer is
***"Which genres has the highest worldwide gross revenue?"***.
...
Most genres have a really low worldwide gross revenue,
...
gross revenue values (but we know there are some since the axis extends that far).
...
it is still not effective in comparing this many distributions
...
from each distribution, such as the mean or median.
...
This is why, in most cases, we need to present a richer representation of the data
...
In this picture, we see that they show the minimum and the maximum values (the range) of our data, as well as 1.5x the interquartile range.

Any observations outside this are drawn out as individual points
...
It becomes harder to compare for the movies from the 3rd position onward.
