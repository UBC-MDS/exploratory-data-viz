...
poorly labelled chart. The y-axis label is not very descriptive, there
...
conveys the main take-home message of our visualization. The y-axis is
...
If you made a more general visualization or answered multiple questions,
...
## Titles can be aligned according to the presentation context
...
## Breaking titles into multiple lines can improve readability
...
When creating a multiline title, it is advisable to try to keep all
lines about the same length. Generally, it also looks better if the
lines are ordered by length, for example from the longest to the
shortest as in this slide.
...
## Direct labelling is often preferred over legend when applicable
...
relying on the legend text and additional coloured line marks.
...
Here we modify the dataframe we created in the last slide by setting the
value for the IBM text label to 140 and the value for the AMZN text
...
title, they can be incorporated directly into each axis label by setting
the axis format.
...
scientific formatting, we can combine the dollar sign with the format
...
`e` to force a scientific format, `d` to force integer format, `~` which
...
that would mean the year “2002.5” and “2007.5”).
...
axis, so we can turn off the vertical gridlines bypassing `grid=False`
...
font sizes to make sure that your figure is readable for the context,
you are presenting it in.
...
In this figure, you can see several examples of how to set font sizes
for titles and labels, as well as how to change the font colour and
weight.
