...
Notes:
In this module we will be learning how to use color and text
to annotate our visualizations
...
we could use color to highlight it.
...
to color by via the `color` encoding.
...
and if it does we use the `'coral'` color,
otherwise we use the default `'steelblue'` color.
...
We need to `alt.value` to tell Altair that we want to use this specific color name,
...
in the same way that we added a custom color.
...
as when we used direct labeling for line graphs
a few slide deck ago.
We add the `text` encoding but the the rest the same
and use `dx` and `dy` to position the label
relative the top of the bar.
...
it is possible to add text annotations directly on the plots.
...
Here we add a thousands separator (`,`)
and round the values to integers (`d`) to remove the decimals.

We also use `dx` to indtroduce some whitespace
...
since this often looks more visually appealing for horizontal bar chart.

We use `dy` to add additional whitespace between the subtitle and the topmost bar
and remove the grey ourline by setting the `strokeWidth` to 0.
...
which automatically changes the color of the y-axis labels and chart title.

Unfortunately,
the subtitle color does not yet change automatically,
so we need to change this manually.

We could have rewritten the entire plot from last slide
...
but in general it is preferred to change these parameters direclty in the main chart code instead
