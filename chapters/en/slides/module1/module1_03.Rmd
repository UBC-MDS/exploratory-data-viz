...
[The Altair documentation includes a table
with all available aggregations](https://altair-viz.github.io/user_guide/encoding.html#binning-and-aggregation).
...
depends on our application and purpose of the data exploration.
...
it is interesting to see how values in the dataframe changes over time.
...
while grouping the cars according their origin,
it is a bit difficult to immediately recognize
which points belongs to which group.
In fact,
...
Altair grammar let's us switch from a point plot to a line plot,
...
Here,
we're showing that this also applies to the color encoding
to illustrate that any encoding
will be propagated to all layers
unless they are specifically overwritten.
If we would only have added color to the point chart,
