...
To colour the points, we need to manually create a sequence of colours
...
code, it is not possible to know which colour represents which country.

We could add a legend by creating it explicitly and adding one coloured
...
the web which are built in Javascript and coloured in white, more on
...
interactive features without reinventing the wheel.
...
frame as a single argument, e.g. `alt.Chart(cars)`.
...
Is there a relationship between horsepower and car weight, or
fuel-efficiency?
...
the categorical column `Origin` (where the car was made).
...
facilitate distinction between the categories. As in the previous slide,
...
we have added the size channel to indicate the engine power.
